...
export default function AgendaSection({ title, data, theme, onChange, onOpenAssignmentPicker, onMaximize, onOpenAI }: any) {
  const editor = useEditor({
    ...
    editorProps: { 
      attributes: { 
        class: 'prose prose-invert focus:outline-none max-w-none text-xl lg:text-2xl min-h-[140px] font-sans leading-relaxed' 
      } 
    },
    onUpdate: ({ editor }) => {
  ...
  // Sync editor if external data changes (like AI generation)
  useEffect(() => {
  ...
  const addMedia = (type: string) => {
    ...
    if (type === 'slides') {
      ...
      if (match) onChange({ ...data, media: { type: 'slides', url: match[0], title: 'Google Slides' } });
    } else {
    ...
    <div className={`group relative flex flex-col p-5 rounded-[2.5rem] border-l-[6px] transition-all duration-700 shadow-2xl overflow-hidden ${theme.card} ${theme.accent}`}>
      ...
      {!data.media && (
        <div className="flex gap-1 mb-3 opacity-0 group-hover:opacity-100 focus-within:opacity-100 transition-opacity bg-black/40 w-fit p-1 rounded-xl border border-white/5 shadow-lg">
          <button onClick={() => editor?.chain().focus().toggleBold().run()} className={`p-1.5 rounded ${editor?.isActive('bold') ? 'bg-[#FCD450] text-black' : 'text-white/40'}`}><Bold size={12}/></button>
          <button onClick={() => editor?.chain().focus().toggleBulletList().run()} className={`p-1.5 rounded ${editor?.isActive('bulletList') ? 'bg-[#FCD450] text-black' : 'text-white/40'}`}><List size={12}/></button>
        </div>
      )}
          ...
          <div className="h-full w-full rounded-3xl overflow-hidden bg-black/40 border border-white/10 shadow-inner">
            ...
            {(data.media.type === 'link' || data.media.type === 'assignment') && (
              <div className="flex flex-col items-center justify-center h-full p-6 text-center">
                <LinkIcon size={40} className="mb-4 opacity-20 text-white" />
                <p className="font-bold text-white mb-4 text-sm truncate w-full">{data.media.title}</p>
                <a href={data.media.url} target="_blank" className="px-6 py-2 bg-white text-black rounded-full font-black text-[10px] uppercase hover:scale-105 transition flex items-center gap-2"><ExternalLink size={12}/> OPEN</a>
              </div>
            )}
          </div>
        ) : (
      ...
      <div className="mt-4 pt-4 border-t border-white/5 flex gap-6 justify-center opacity-0 group-hover:opacity-100 transition-all duration-300 transform translate-y-2 group-hover:translate-y-0">
        <button onClick={() => addMedia('image')} className="text-white/20 hover:text-white flex flex-col items-center gap-1 transition"><ImageIcon size={18}/><span className="text-[7px] font-black uppercase tracking-tighter">Image</span></button>
        <button onClick={() => addMedia('slides')} className="text-white/20 hover:text-white flex flex-col items-center gap-1 transition"><Presentation size={18}/><span className="text-[7px] font-black uppercase tracking-tighter">Slides</span></button>
        <button onClick={onOpenAssignmentPicker} className="text-white/20 hover:text-emerald-400 flex flex-col items-center gap-1 transition"><GraduationCap size={18}/><span className="text-[7px] font-black uppercase tracking-tighter">Task</span></button>
      </div>
    </div>
  );
}